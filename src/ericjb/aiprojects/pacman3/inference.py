...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        for p in self.legalPositions:
            pDist = util.manhattanDistance(p, pacmanPosition)
            newObs = emissionModel[pDist]
            prev = self.beliefs[p]
            self.beliefs[p] = (newObs*prev)
        self.beliefs.normalize()
        "*** YOUR CODE HERE ***"
    ...
    def elapseTime(self, gameState):
        ...
        newPostDist[p] = P( ghost is at position p at time t + 1 | ghost is at position oldPos at time t )
        ...
        for key in self.beliefs:
            if self.beliefs[key] != 0:
                oldPos[key] = self.beliefs[key]
        newPos = util.Counter({})
        for key in oldPos:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, key))
            for pos in newPosDist:
                newPos[pos] += (newPosDist[pos]*oldPos[key])
        newPos.normalize()
        self.beliefs = newPos
        
            
        #Get all of the positions the ghost could have been in, 
        #from each position get the probability of the ghost being in all new positions,
        #Multiply each probability of the new positions by the probability of the old position
