...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        mostLikely = []
        for ghostIndex in livingGhostPositionDistributions:
            big = 0
            for key in ghostIndex:
                if ghostIndex[key] > big:
                    big = ghostIndex[key]
                    best = key
            mostLikely.append(best)
        small = 99
        for pos in mostLikely:
            new = self.distancer.getDistance(pacmanPosition, pos)
            if new < small:
                closeGhost = pos
                small = new
        small = 99
        for action in legal:
            ...
            new = self.distancer.getDistance(successorPosition, closeGhost)
            if new < small:
                bestMove = action
                small = new
        return bestMove
